...
pub enum ZeroBlocks {
    ...
    Include,
    /// Omit blocks whose content is entirely zeroes (a full content scan).
    /// NOT safe for a snapshot that will be launched as an ENCRYPTED volume:
    /// omitted blocks are synthesized by EBS as decrypt(zeroes) = garbage, and
    /// this mode drops allocated-zero blocks that the guest reads back
    /// expecting zeroes, corrupting the volume. Use only for unencrypted use.
    Omit,
    /// Omit only never-written filesystem holes (sparse regions), detected via
    /// `SEEK_DATA`/`SEEK_HOLE`. Blocks explicitly written with zeroes are still
    /// uploaded. Unlike `Omit`, this IS safe for a snapshot launched as an
    /// ENCRYPTED volume: only never-written free space is omitted (the guest
    /// does not read it back as meaningful data), while every allocated block,
    /// including written zeroes, is uploaded. If the source is a block device
    /// or lives on a filesystem without hole support, no blocks are omitted.
    OmitHoles,
}

impl ZeroBlocks {
    /// Whether filesystem holes should be omitted for this mode. Holes are
    /// skipped for both `Omit` (a read-avoidance fast path, since a hole always
    /// reads back as zeroes) and `OmitHoles` (the sole omission criterion).
    fn omits_holes(self) -> bool {
        matches!(self, ZeroBlocks::Omit | ZeroBlocks::OmitHoles)
    }

    /// Whether a block whose *content* is entirely zeroes should be omitted
    /// after it is read. Only `Omit` performs this content scan; `OmitHoles`
    /// keeps explicitly-written zero blocks.
...
impl SnapshotUploader {
    ...
    pub async fn upload_from_file<P: AsRef<Path>>(
        ...
        // When omitting zero/hole blocks, precompute the file's data-extent map
        // once via SEEK_DATA/SEEK_HOLE. For `Omit` this is a fast path that lets
        // workers skip known holes without reading them; for `OmitHoles` it is
        // the sole omission criterion. Block devices and filesystems without
        // hole support fall back to a "full" map, so nothing is skipped as a
        ...
        let data_map = if zero_blocks.omits_holes() {
            ...
            if let Some(sidecar) = sidecar_path.as_ref() {
                ...
                // A hard failure to open or scan the source is not recoverable
                // by "upload everything": the per-block workers open the same
                // file and would fail identically, so propagate it instead of
                // masking it behind a full-map fallback. An unsupported
                // filesystem (Ok(None)) is a real, expected fallback, not error.
                let scanned =
    ...
    async fn upload_block(&self, context: &BlockContext) -> Result<()> {
        ...
        // Hole fast-path: when omitting zero/hole blocks, skip any block that
        // lies entirely within a filesystem hole without reading it from disk.
        // For `Include` the map is "full" so this never triggers. On an
        // unencrypted snapshot a hole reads back as zeroes, so omitting it does
        // not change the reconstructed image; the encryption caveat is the same
        // as for content-based omission.
